<div class="d-inline-block">
    <button type="button" class="btn btn-sm btn-danger text-white" data-bs-toggle="modal" data-bs-target="#modal-forceDelete-teacher-id{{ $this->teacher->id }}">
        <i class="fa fa-trash"></i>
    </button>

    <div wire:ignore.self class="modal fade" id="modal-forceDelete-teacher-id{{ $this->teacher->id }}" tabindex="-1" role="dialog" aria-labelledby="modal-forceDelete-teacher-id{{ $this->teacher->id }}" aria-hidden="true">
                ...
                <form wire:submit.prevent="forceDelete" class="form-border-radius">
                    <div class="block block-rounded shadow-none mb-0">
                        <div class="block-header block-header-default">
                            <h3 class="block-title fs-sm mt-1">{{ trans('dashboard.sections.teacher') }} {{ strtolower(trans('dashboard.actions.force-delete')) }}</h3>
